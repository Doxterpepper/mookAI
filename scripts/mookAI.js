...
export class MookAI {
	...
	async takeNextTurn() {
		...
		const success = await this.takeMookTurn(this.getMook(this.getCombat(), game.combat.current.tokenId));
	...
	updateTokens(changes_) {
		this.combats.forEach(mooks => {
			mooks.forEach(m => {
				m.handleTokenUpdate(changes_);
			});
		});
	}

	get autoEndTurn() {
		return game.settings.get('mookAI', 'AutoEndTurn');
	}
