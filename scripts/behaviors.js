...
export class Behaviors {
    ...
    static attackByDistance(mook_, targets_, gmTech_) {
        ...
        if (mm.hasMele && targets_.mele.length > 0) {
            const token = func(targets_.mele, t => mook_.pathManager.path(mook_.token.id, t.id).cost);
            return new Target(token, mm.meleRange, mm.meleAttackAction());
        }

        if (mm.hasRanged && targets_.ranged.length > 0) {
            const token = func(targets_.ranged, t => mook_.pathManager.path(mook_.token.id, t.id).cost);
