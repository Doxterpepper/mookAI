...
export class Mook {
    ...
    async sense() {
        ...
        //
        // The pathManager holds the potential paths to a target. These are caclulated with A* path search.
        //
    ...
    planTurn() {
        ...
        //
        // There's a time involved with acting. If the time is up, it looks like we just halt and end the mooks turn. Otherwise, the mook tries to explore.
        //
        ...
        // Select the target based on how this mook is supposed to behave. Could find the closest, could find the one with the lowest health.
        ...
        //
        // Look to see if there is a path to the target.
        //
        const path = this.pathManager.path(this.token.id, target.id);

        //
        // Note: What's the point of a path that isn't valid?
        //
        if (path.valid) {
            this.plan.push({
            ...
            //
            // Note: How can we traverse a null path?
            //
            this.plan.push({
                actionType: ActionType.TRAVERSE,
                cost: 0,
    ...
    async act() {
        ...
        while (this.time >= 0 && --tries) {
            ...
            //
            // Actions are a FIFO queue. Inspect the front item.
            //
            const action = this.plan.splice(0, 1)[0];
    ...
    get viableTargets() {
        ...
        if (this.mookModel.hasMele) {
            meleTargets = this.visibleTargets.filter(e => this.isTargetReachable(e, this.mookModel.meleRange));
        }

        if (this.mookModel.hasRanged) {
            rangedTargets = this.visibleTargets.filter(e => this.isTargetReachable(e, this.mookModel.rangedRange));
        }
	...
	**/
    ...
    async step() {
        const angles = this.neighborAngles.sort((a, b) => Math.min(a, 360 - a) - Math.min(b, 360 - b));
    ...
    get tokenDoc() {
        return game.scenes.active.tokens.get(this._token.id);
    }
